...
var vm = new Vue({
  ...
  methods: {
    ...
    optionSelect: function optionSelect(res) {
      vm.districtFilter = res;
      SelectShow();
    },
    petSelect: function petSelect(res) {
      this.PetFilter = res;
    },
    isOpenSlect: function isOpenSlect(res) {
      this.isOpenFilter = res;
    },
    filter: function filter(res) {
      this.filterData = this.vetData.filter(function (obj) {
  ...
  computed: {
    filterData: function filterData() {
      ...
      if (District != "全部") {
        filterArry = filterArry.filter(function (obj) {
      ...
      vm.pageNum = 1;
      return filterArry.sort(function (a, b) {
...
$.ajax({
  ...
  success: function success(res) {
    vm.vetData = res.feed.entry.map(function (obj) {
      return {
        name: obj.gsx$醫院名稱.$t,
    ...
    vm.vetData.forEach(function (obj) {
...
function isOpen(res) {
  ...
  var ans = Comparison.some(function (obj3) {
    return obj3 == true;
  });
...
function detailMap() {
  ...
  var map = new google.maps.Map(document.getElementById("map"), {
    center: {
      lat: Number(mapData.positioning[0]),
      lng: Number(mapData.positioning[1])
    },
    zoom: 18
  });
